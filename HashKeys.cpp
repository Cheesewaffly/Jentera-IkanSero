...
U64 generatePositionKeys(const Board *position){ //Generates a unique key for each position
    ...
    for(int square120Index = 0; square120Index < boardSquareNumber; ++square120Index){ //Loops through all the squares on the 120 array board
        int piece = position->boardPieces[square120Index]; //The variable piece is equal to whatever the value the current square is
        if(piece != emptySquare && piece != emptyPiece){ //Checks wether the square is a border square or an empty square
